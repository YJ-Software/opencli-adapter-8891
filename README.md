...
# 2. Clone 並安裝轉接器套件（repo 中已包含預編譯的 .js 檔案）
...
# 3. 驗證 — 應該看到 3 個命令，且沒有「Ignoring TypeScript adapter」警告
...
若修改了 `clis/8891/list.ts` / `detail.ts` / `electric.ts`，需重新建置並安裝：

```bash
cd ~/opencli-adapters/clis/8891
npm install           # 首次執行，安裝 typescript + @types/node
npm run build         # tsc 編譯 .ts → .js（放在原始碼旁邊）
npm run install-local # 將 .js + brands.json + db/ 複製到 ~/.opencli/clis/8891/
```
...
| `opencli 8891 electric` | 純電車列表快捷指令（保留供向下相容） |
...
參見 [`clis/8891/db/`](clis/8891/db/) — 一個 Python 同步腳本，將 OpenCLI 的輸出持久化到 SQLite，以便執行歷史查詢。**跨平台**：純 Python 標準函式庫，已在 Windows 測試，Linux/macOS 無需修改即可運作。
...
完整文件請見 [`clis/8891/db/README.md`](clis/8891/db/README.md)，13 個現成的 SQL 查詢範例請見 [`clis/8891/db/queries.sql`](clis/8891/db/queries.sql)（最大折扣、降價追蹤、瀏覽數增長、品牌統計等）。
